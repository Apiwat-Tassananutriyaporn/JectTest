# 06 Requirement - 3D SCADA Web Platform

## Version Update Frontend
- Version นี้ update จาก `C:\JectTest\05-requirement.md`
- ไฟล์นี้เป็น requirement ฉบับ version 06 สำหรับโปรเจกต์ `C:\JectTest`
- การเปลี่ยนแปลงหลักจาก version 05:
...
## 2.1 Implementation Scope ของ Version 06

Version 06 เป็น frontend-only prototype

ต้องทำใน phase นี้:

- React + Vite + TypeScript frontend
- UI สำหรับ Project Settings, Scene Editor, Tag Config, PLC Config, Alarm Log, Dashboard
...
- PLC Gateway JSON tag จาก MQTT sensors topic
- PLC command publish ผ่าน MQTT command topic
- PLC gateway status ผ่าน MQTT status topic
...
- MQTT TCP gateway
- Modbus TCP/RTU
- Direct PLC protocol integration
...
Backend Gateway เป็น future/production phase ไม่ใช่ scope ที่ต้อง implement ใน version 06
...
- Version 06 ยังไม่ต้องทำ backend gateway
- Version 06 ใช้ MQTT over WebSocket จาก frontend/browser โดยตรงก่อน
...
- Version 06 ยังไม่ต้องมี REST API จริง
- Version 06 ยังไม่ต้องมี backend WebSocket จริง
...
- จุดประสงค์คือให้ลอง UI, scene layout, tag config, PLC config และ dashboard ได้ก่อน
...
Version 06 ให้ implement frontend ก่อน
...
Future/production scope ยังไม่ต้อง implement ใน version 06
...
- Version 06 ยังไม่ต้องสร้าง backend project
- Version 06 ยังไม่ต้องติดตั้ง Prisma/PostgreSQL/TimescaleDB
- Version นี้ยังไม่รองรับ Modbus TCP/RTU
...
```text
apps/api/src/   # future, not required in version 06
  projects/
  auth/        # future
  users/       # future
  mqtt/
...
Acceptance criteria สำหรับ version 06 ให้เน้น frontend prototype ก่อน
...
- Version 06 ยังไม่ต้องใช้ backend gateway
...
- สามารถสร้าง/แก้ไข PLC config ใน frontend ได้
...
Version 06 must connect to MQTT broker directly from the browser using MQTT over WebSocket (`ws://` or `wss://`). If a broker only supports MQTT TCP (`mqtt://host:1883`), it is out of scope for this version and requires a future backend gateway. Modbus TCP/RTU is not supported in version 06 and should be treated as a future feature.

Version 06 must support PLC data through an ESP gateway using MQTT topic/payload patterns:
...
- PLC gateway MQTT sensor/command/status support without Modbus TCP/RTU connection in version 06
